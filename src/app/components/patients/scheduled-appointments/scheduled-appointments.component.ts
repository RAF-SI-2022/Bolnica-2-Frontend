...
export class ScheduledAppointmentsComponent implements OnInit {
  ...
  getScheduledAppointmentsForDoctor() {
    ...
    this.scheduledAppointmentService.getScheduledAppointments(formattedDate, this.page - 1, this.pageSize).subscribe({
      next: (res) => {
  ...
  getScheduledAppointmentsForNurse() {
    ...
    const formattedDate = `${day}/${month}/${year}`;
    this.currentDate = formattedDate;
    this.scheduledAppointmentService.getScheduledAppointmentsByLbz(formattedDate, this.doctorLbz, this.page - 1, this.pageSize).subscribe({
      next: (res) => {
        this.schedMedExamsList = res.schedMedExamResponseList;
        this.collectionSize = res.count;
      },
